...
summary:    "《算法笔记》第 4 篇：弹簧一边伸缩、一边摆动，轨迹该怎么算？把系统的能量写成运动方程，再让数值计算还原它的运动。"
series:     算法笔记
series_index: 4
---
...
<svg id="spring-pendulum-svg" width="400" height="400" viewBox="-200 -100 400 400" aria-label="弹簧摆运动模拟"></svg>
<button id="pause-btn" class="pause-btn" style="margin-top: 10px;">暂停</button>
<p id="simulation-status" role="status" aria-live="polite"></p>
```
...
const inputs = [LprimeInput, thetaSlider, L0Input, kInput, mInput, dtInput];

function checkRadius(radius) {
    if (!Number.isFinite(radius) || radius <= 0.01) {
        throw new Error('质点接近悬点，极坐标方程不再稳定。请调整初始角度或其他参数。');
    }
    if (radius > maximumRadius * 1.05) {
        throw new Error('数值误差已超出当前能量允许的运动范围。请减小时间步长或调整参数。');
    }
}

function simulateSpringPendulum(dt) {
    checkRadius(L0 + L_prime);
    const spring_force1 = L_prime > 0 ? (k / m) * L_prime : 0;
    const k1_L = v;
    const k1_v = (L0 + L_prime) * omega * omega + g * Math.cos(theta) - spring_force1;
    const k1_theta = omega;
    ...
    checkRadius(L0 + L_prime + 0.5 * k1_L * dt);
    const k2_L = v + 0.5 * k1_v * dt;
    const spring_force2 = (L_prime + 0.5 * k1_L * dt) > 0 ? (k / m) * (L_prime + 0.5 * k1_L * dt) : 0;
    const k2_v = (L0 + L_prime + 0.5 * k1_L * dt) * (omega + 0.5 * k1_omega * dt) ** 2
                  + g * Math.cos(theta + 0.5 * k1_theta * dt)
    ...
    checkRadius(L0 + L_prime + 0.5 * k2_L * dt);
    const k3_L = v + 0.5 * k2_v * dt;
    const spring_force3 = (L_prime + 0.5 * k2_L * dt) > 0 ? (k / m) * (L_prime + 0.5 * k2_L * dt) : 0;
    const k3_v = (L0 + L_prime + 0.5 * k2_L * dt) * (omega + 0.5 * k2_omega * dt) ** 2
                  + g * Math.cos(theta + 0.5 * k2_theta * dt)
    ...
    checkRadius(L0 + L_prime + k3_L * dt);
    const k4_L = v + k3_v * dt;
    const spring_force4 = (L_prime + k3_L * dt) > 0 ? (k / m) * (L_prime + k3_L * dt) : 0;
    const k4_v = (L0 + L_prime + k3_L * dt) * (omega + k3_omega * dt) ** 2
                  + g * Math.cos(theta + k3_theta * dt)
...
function drawSpringPendulum(L_prime, theta) {
    ...
    const L_total = L0 + L_prime;
    const x_m = L_total * Math.sin(theta);
    const y_m = L_total * Math.cos(theta);
    /* 只扩大视野，不修改物理坐标；重设参数时恢复初始视野。 */
    viewRadius = Math.max(viewRadius, L_total + 24);
    svg.setAttribute("viewBox", `${-viewRadius} ${-viewRadius} ${viewRadius * 2} ${viewRadius * 2}`);
    ...
    /* 绘制弹簧 */
    const spring = document.createElementNS("http://www.w3.org/2000/svg", "line");
    spring.setAttribute("x1", 0);
    spring.setAttribute("y1", 0);
    spring.setAttribute("x2", x_m);
    spring.setAttribute("y2", y_m);
...
function animate() {
    const previous = [L_prime, v, theta, omega];
    try {
        /* 保留同一个 RK4 方程，以更小的子步走完用户指定的时间。 */
        const steps = Math.ceil(dt / 0.02);
        for (let i = 0; i < steps; i++) {
            simulateSpringPendulum(dt / steps);
            if (![L_prime, v, theta, omega].every(Number.isFinite)) {
                throw new Error('数值计算不再稳定。请减小时间步长或调整参数。');
            }
            checkRadius(L0 + L_prime);
        }
        drawSpringPendulum(L_prime, theta);
        ...
        [L_prime, v, theta, omega] = previous;
        drawSpringPendulum(L_prime, theta);
        failed = true;
        isPaused = true;
...
function resetAnimation() {
    ...
    if (inputs.some(input => !input.checkValidity() || input.value === '')) {
        status.textContent = '请在标示范围内填写所有参数。';
    ...
    m = parseFloat(mInput.value);
    dt = parseFloat(dtInput.value);
    /* E = 动能 + 弹性势能 - m g r cos(theta)。由势能下界求最大可能半径。 */
    const energy = 0.5 * k * L_prime ** 2 - m * g * (L0 + L_prime) * Math.cos(theta);
    const extension = m * g / k;
    maximumRadius = L0 + extension + Math.sqrt(Math.max(0, extension ** 2 + 2 * (energy + m * g * L0) / k));
    viewRadius = 190;
    drawSpringPendulum(L_prime, theta);
...
> 画面会随运动范围扩展，缩放不会改变物理坐标。这里使用极坐标求解：当质点接近悬点、半径趋近零时，方程会出现奇点，演示会暂停并提示调整参数；数值发散时同样会停止，而不是把位置强行限制在画布边缘。
