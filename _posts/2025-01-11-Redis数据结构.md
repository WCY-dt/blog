...
在[初始化字符串](https://github.com/redis/redis/blob/7.4/src/sds.c#L81)时，Redis 会首先调用 [`sdsReqType` 函数](https://github.com/redis/redis/blob/7.4/src/sds.c#L38)，根据字符串长度决定字符串类型，然后分配空间并初始化字符串。
...
对于 `sds`，Redis 会根据字符串长度调整空间大小。这个过程在 [`_sdsMakeRoomFor` 函数](https://github.com/redis/redis/blob/7.4/src/sds.c#L217)中实现。
...
在运行过程中，Redis 为了避免挤爆 `hashtable`，会判断[是否需要扩容](https://github.com/redis/redis/blob/7.4/src/dict.c#L1492)。
...
扩容后的大小通过[函数计算](https://github.com/redis/redis/blob/7.4/src/dict.c#L1560)得到。具体来讲，如果扩容后大小不超过 `long` 类型的最大值，那么 `hashtable` 的大小会以 $$2$$ 的幂次方增长。
...
如果满足了扩容条件，Redis 会最终调用到 [`dictRehash` 函数](https://github.com/redis/redis/blob/7.4/src/dict.c#L385)来扩容 `hashtable`。
...
当全部数据迁移完成后，Redis 会[将扩容后的 `hashtable` 转正](https://github.com/redis/redis/blob/7.4/src/dict.c#L362)。也就是说，将 `ht_table[0]` 和 `ht_table[1]` 的指针互换，释放 `ht_table[0]` 的内存，并将 `rehashidx` 重置为 $$-1$$。
...
如果哈希表太空了，Redis 也会[缩容](https://github.com/redis/redis/blob/7.4/src/dict.c#L1529)。
...
Redis 会根据元素值的大小选择合适的编码。`intset` 的[编码](https://github.com/redis/redis/blob/7.4/src/intset.c#L41)有三种：
...
添加的过程是通过将大于要添加的值的元素后移一个位置，然后将新元素插入到空出的位置，类似插入排序。这个过程在 [`intsetAdd` 函数](https://github.com/redis/redis/blob/7.4/src/intset.c#L206)中实现。
...
如果元素的大小超过了当前编码的范围，Redis 会将 `intset` 的编码升级为更大的编码。`intset` 的编码升级是通过 [`intsetUpgradeAndAdd` 函数](https://github.com/redis/redis/blob/7.4/src/intset.c#L159)实现的。它分为三步操作：
...
其实现在 [`zslRandomLevel`](https://github.com/redis/redis/blob/7.4/src/t_zset.c#L126)：
...
`zlentry` 的[定义](https://github.com/redis/redis/blob/7.4/src/ziplist.c#L283)如下：
...
为了将 `zlentry` 合并到一起，`ziplist` 整体在内存中按照[以下方式](https://github.com/redis/redis/blob/7.4/src/ziplist.c#L711)存储。
...
`encode & len` 很容易得到，它们两个确定了 `content` 的长度。Redis 将这两者编码在了一起。根据元素类型和长度的不同，有不同的[计算方法](https://github.com/redis/redis/blob/7.4/src/ziplist.c#L368)，来确定放在内存中的值：
...
而 `prevrawlen` 则是在添加元素时更新的。其[计算方法](https://github.com/redis/redis/blob/7.4/src/ziplist.c#L443)如下：
...
但是问题来了，对于反向遍历，`listpack` 由于不记录前一个节点的长度，怎么办呢？[解决](https://github.com/redis/redis/blob/7.4/src/listpack.c#L473)这个事情也不难：
...
这里的关键在于如何[计算前一个条目长度](https://github.com/redis/redis/blob/7.4/src/listpack.c#L374)。`listpack` 采用了一种变长编码的方式：
...
其[实现](https://github.com/redis/redis/blob/7.4/src/hyperloglog.c#L1035)为：
