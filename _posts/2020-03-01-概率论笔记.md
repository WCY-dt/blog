...
categories: 数学
tags:       note probability
summary:    "本文为概率论笔记，内容包括随机事件及概率、随机变量及其概率分布、随机向量及其概率分布等基础知识，帮助读者理解和掌握概率论的基本概念和方法。"
---
...
- ***联合分布函数***
  ...
  - $$\begin{aligned}&P\left(x_1<X\leq x_2,y_1<Y\leq y_2\right)\\=&F\left(x_2,y_2\right)-F\left(x_2,y_1\right)+F\left(x_1,y_1\right)-F\left(x_1,y_2\right)\end{aligned}$$​
...
- ***随机变量函数的数学期望***
  ...
  - 设 $$Y=g\left(X\right)$$
    ...
    - 若 $$X$$ 为连续型随机变量，其密度函数为 $$f\left(x\right)$$，则
...
- ***$$n$$ 维正态分布***
  ...
  正定, $$\lvert \Sigma\rvert $$ 是 $$\Sigma$$ 的行列式，则称 $$X$$ 服从 $$n$$ 维正态分布，记为
  
  $$
  X\sim N\left(\mu_{1\times n},\Sigma_{n\times n}\right)
  $$
...
- ***辛钦大数定律***
  ...
  $$\left\{X_n\right\}$$ 独立同分布，$$EX_n=\mu\left(n=1,2,\cdots\right)$$ 存在，则
  
  $$
  \forall \varepsilon>0,\lim_\limits{n\rightarrow \infty}{P\left(\lvert \frac{1}{n}\sum_\limits{k=1}^{n}{X_k}-\mu\rvert \geq\varepsilon\right)}=0
  $$
...
- ***定义***
  ...
  相互独立的随机变量序列 $$\left\{X_n\right\}$$，设 $$EX_n,DX_n\left(n=1,2,\cdots \right)$$ 存在，令
  
  $$
  Y_n=\frac{\sum_\limits{i=1}^{n}{EX_i}-\sum_\limits{i=1}^{n}{X_i} }{\sqrt{\sum_\limits{i=1}^{n}{DX_i} }}
  $$
  
  若
  
  $$
  \lim_\limits{n\rightarrow \infty}{P\left\{Y_n\leq x\right\} }=\Phi\left(x\right)=\frac{1}{\sqrt{2\pi} }\int_{-\infty}^{x}{e^{-\frac{t^2}{2} }\mathrm{d}t}
  $$
  
  则称 $$\left\{X_n\right\}$$ 服从中心极限定理

- ***林德贝格定理***

  设 $$\left\{X_n\right\}$$ 相互独立，数学期望和方差存在
  
  $$
  EX_k=\mu_k,DX_k=\sigma_{k}^2\left(k=1,2,\cdots,n,\cdots\right)
  $$
  
  记 $$B_n^2=\sum_\limits{k=1}^{n}{\sigma_{k}^2}$$，若 $$\forall\varepsilon>0$$，有
  
  $$
  \lim_\limits{n\rightarrow \infty}{\frac{1}{B^2}\sum_\limits{k=1}^{n}{\int_{\lvert x-\mu_k\rvert \geq\varepsilon B_n}{ {\left(x-\mu_k\right)}^2\mathrm{d}{F_k\left(x\right)} }} }=0
  $$
  
  则 $$\left\{X_n\right\}$$ 服从中心极限定理
...
- ***独立同分布的中心极限定理***
  ...
  $$
  EX_k=\mu,DX_k=\sigma^2<+\infty\left(k=1,2,\cdots\right)
  $$
  
  则 $$\forall\varepsilon\in \mathbb{R}$$，
...
- ***统计量***
  ...
  - 常用统计量
    ...
    - 样本 $$k$$ 阶原点矩
      ...
      $$
      A_k=\frac{1}{n}\sum_\limits{i=1}^{n}{X_i^k},k=1,2,\cdots
      $$

    - 样本 $$k$$ 阶中心矩

      $$
      B_k=\frac{1}{n}\sum_\limits{i=1}^{n}{ {\left(X_i-\overline{X}\right)}^k},k=2,3,\cdots
      $$
      ...
      - 样本的联合分布
        ...
        - 若 $$X\sim F\left(x\right)$$,$$X_1,X_2,\cdots,X_n$$ 为 $$F$$ 的一个样本，则 $$X_1,X_2,\cdots,X_n$$ 的联合分布函数

          $$
          F^{*}\left(x_1,x_2,\cdots,x_n\right)=\prod_\limits{i=1}^{n}{F\left(x_i\right)}
          $$

        - 若总体 $$X$$ 是离散型随机变量，其分布律为 $$p_x=P\left(X=x\right),x=x_1,x_2,\cdots$$，则 $$X_1,X_2,\cdots,X_n$$ 的联合分布函数为
...
- ***单个正态总体***
  ...
  设 $$\left(X_1,\cdots,X_n\right)$$ 为来自总体 $$N\left(\mu,{\sigma}^2\right)$$ 的一组容量为 $$n$$ 的样本，令
  
  $$
  \overline{X}=\frac{1}{n}\sum_\limits{i=1}^{n}{X_i},S^2=\frac{1}{n-1}\sum_{i=1}^{n}{ {\left(X_i-\overline{X}\right)}^2}
  $$
...
- ***两个正态总体***
  ...
  设 $$\left(X_1,\cdots,X_m\right)$$ 为来自总体 $$X\sim N\left(\mu_1,{\sigma}_1^2\right)$$ 的一组容量为 $$m$$ 的样本，$$\left(Y_1,\cdots,Y_n\right)$$ 为来自总体 $$Y\sim N\left(\mu_2,{\sigma}_2^2\right)$$ 的一组容量为 $$n$$ 的样本，两组样本相互独立，令
  
  $$
  \begin{aligned}
  &\overline{X}=\frac{1}{m}\sum_\limits{i=1}^{m}{X_i}\\
  &S_{1m}^2=\frac{1}{m-1}\sum_{i=1}^{m}{ {\left(X_i-\overline{X}\right)}^2}\\
  &\overline{Y}=\frac{1}{n}\sum_\limits{i=1}^{n}{Y_i}\\
  &S_{2n}^2=\frac{1}{n-1}\sum_{i=1}^{n}{ {\left(Y_i-\overline{Y}\right)}^2}
  \end{aligned}
  $$
...
- ***矩估计法***
  ...
  设总体 $$X$$ 的分布函数为 $$F\left(x;\theta_1,\theta_2,\cdots,\theta_k\right)$$，称
  ...
  的解 $$\hat{\theta}\left(X_1,X_2,\cdots,X_n\right)$$ 为 $$\hat{\theta}\left(x_1,x_2,\cdots,x_n\right)$$ 的矩估计量
...
- ***极大似然估计法***
  ...
  - 求解方法：
    ...
    - 求解对数似然方程
      ...
      $$
      \frac{\partial\ln{L\left(\theta_1,\theta_2,\cdots,\theta_k\right)} }{\partial\theta_i}=0\left(i=1,2,\cdots,k\right)
      $$
...
- ***求置信区间（枢轴量法）***
  ...
  2. 对于给定的置信度 $$1-\alpha$$，求出 $$a,b$$，使得
  
     $$
     P\left\{a<Z\left(X_1,\cdots,X_n;\theta\right)<b\right\}=1-\alpha
     $$

  3. 由不等式 $$a<Z\left(X_1,\cdots,X_n;\theta\right)<b$$ 解得
  
     $$
     \hat{\theta}_1\left(X_1,\cdots,X_n\right)<\theta<\hat{\theta}_2\left(X_1,\cdots,X_n\right)
     $$

     即 $$P\left(\hat{\theta}_1<\theta<\hat{\theta}_2\right)=1-\alpha$$
...
- 一般步骤
  ...
  - 对给定的显著性水平 $$0<\alpha<1$$，根据
  
    $$
    P\left(\left(X_1,X_2,\cdots,X_n\right)\in S\mid H_0\right)=\alpha
    $$
...
- $$\sigma^2$$ 已知，检验 $$\mu$$
  ...
  - $$H_0:\mu=\mu_0,H_1:\mu\neq\mu_0$$，双边检验
    ...
    - 令 $$k=u_{\frac{\alpha}{2} }$$，最大允许拒绝域为

      $$
      S=\left\{\left(x_1,\cdots,x_n\right)\mid \lvert \frac{\overline{X}-\mu_0}{\sigma}\sqrt{n}\rvert \geq u_{\frac{\alpha}{2} }\right\}
      $$
  ...
  - $$H_0:\mu\leq\mu_0,H_1:\mu>\mu_0$$，单边右检验
    ...
    - 当 $$\mu\leq\mu_0$$ 时，统计量
      ...
      $$
      U=\frac{\overline{X}-\mu}{\sigma}\sqrt{n}\sim N\left(\frac{\mu-\mu_0}{\sigma}\sqrt{n},1\right)
      $$

      分布已知

    - $$\begin{aligned}&P\left(\frac{\overline{X}-\mu_0}{\sigma}\sqrt{n}\geq k\mid H_0\right)\\=&P\left(\frac{\overline{X}-\mu}{\sigma}\sqrt{n}\geq k-\frac{\mu-\mu_0}{\sigma}\sqrt{n}\mid H_0\right)\\\leq&P\left(\frac{\overline{X}-\mu}{\sigma}\sqrt{n}\geq k\mid H_0\right)\\\leq&\alpha\end{aligned}$$​

      满足该不等式则为 $$H_0$$ 的拒绝域

    - 令 $$k=u_{\alpha}$$，最大允许拒绝域为

      $$
      S=\left\{\left(x_1,\cdots,x_n\right)\mid \lvert \frac{\overline{X}-\mu_0}{\sigma}\sqrt{n}\rvert \geq u_{\alpha}\right\}
      $$
