...
categories: 数据库
tags: mysql sql db
summary: "本文介绍了 MySQL 的实现原理，包括表的结构设计、执行流程、事务的实现、索引的使用、锁的机制等。通过理解这些原理，可以更好地使用和优化 MySQL 数据库。"
comments: true
copyrights: 原创
mathjax: true
mermaid: true
...
<img src="/assets/post/images/mysql7.svg" alt="Read View"  style="width:min(100%,250px);" />
...
- Free 链表：用于存储空闲的页。当 Buffer Pool 中有新的页需要分配时，会从 Free 链表中取出一个页
