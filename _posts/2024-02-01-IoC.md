...
它会收先调用 `AnnotationConfigApplicationContext()` 构造函数：
...
`scan()` 方法会调用 `ClassPathBeanDefinitionScanner` 的 `scan()` 方法：
...
protected Set<BeanDefinitionHolder> doScan(String... basePackages) {
		Assert.notEmpty(basePackages, "At least one base package must be specified");
    // BeanDefinitions 使用了一个 LinkedHashSet 来存储，每个元素都包含了 BeanDefinition 和 Bean 名称的二元组
    ...
    // 遍历需要扫描的包
        ...
        for (BeanDefinition candidate : candidates) {
            ...
            // 生成 Bean 名称，如果制定了 Bean 名称则使用指定的名称
            String beanName = this.beanNameGenerator.generateBeanName(candidate, this.registry);
            // 处理 lazy、Autowire、DependencyOn、initMethod、enforceInitMethod、destroyMethod、enforceDestroyMethod、Primary、Role、Description
            ...
            if (checkCandidate(beanName, candidate)) {
                // 形成 BeanDefinition, beanName 二元组
                BeanDefinitionHolder definitionHolder = new BeanDefinitionHolder(candidate, beanName);
                definitionHolder =
                    AnnotationConfigUtils.applyScopedProxyMode(scopeMetadata, definitionHolder, this.registry);
                // 将二元组加入到 BeanDefinitions 中
...
- `findCandidateComponents()` 方法：
  ...
  继续跟踪 `scanCandidateComponents()` 方法：

  ```java
  private Set<BeanDefinition> scanCandidateComponents(String basePackage) {
      Set<BeanDefinition> candidates = new LinkedHashSet<>();
      try {
          // 扫描路径类似 classpath*:com/example/**/*.class */
          ...
          for (Resource resource : resources) {
              ...
              try {
                  // 通过反射获取 MetadataReader
                  MetadataReader metadataReader = getMetadataReaderFactory().getMetadataReader(resource);
                  // 查看配置类是否有 @Conditional 注解
...
- `registerBeanDefinition()` 方法：
  ...
  `registerBeanDefinition()` 是个接口，我们这里用的 `DefaultListableBeanFactory` 的实现：
  ...
  @Override
	public void registerBeanDefinition(String beanName, BeanDefinition beanDefinition)
      ...
      // 检查是否已经在 beanDefinitionMap 中存在（即是否已经注册过）
      BeanDefinition existingDefinition = this.beanDefinitionMap.get(beanName);
      // 处理同名的 BeanDefinition
      if (existingDefinition != null) {
          ...
          if (hasBeanCreationStarted()) { // 如果已经有其它的 Bean 在初始化了
              // 这里是线程安全的
